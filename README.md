...
- **Proactive injection (learn & prevent)** — Optionally, instead of only fixing replies after the fact, Phrase Ban keeps a **per-chat learned list** of every phrase it detects and persistently injects a *"don't reuse these phrases"* instruction before every AI turn — so future replies avoid the tics up front. Pair it with **Max Rewrite Attempts = 0** for pure prevention (learn + warn, never rewrite). The injection's depth and role are configurable, and the list travels with the chat. The learned list is fully **editable** in a per-chat textarea — add your own phrases or delete ones you don't want injected — with a **Clear Learned Phrases** button to wipe it. When Proactive is off, nothing is learned or injected and Phrase Ban behaves exactly as the reactive rewrite path.
- **Hard Token Ban (sampler-level)** — A step up from the soft injection: on **Text Completion** backends (llama.cpp, KoboldCpp, TabbyAPI/ExLlama, etc.), the learned list is appended to the request's `banned_strings`, so the backend *refuses to emit* those sequences at the sampler level instead of merely being asked to. It's applied non-destructively per-request (your saved sampler settings are never touched) and, on those backends, replaces the soft prompt injection to save context tokens. Chat Completion APIs (OpenAI, Claude, …) have no sampler-level ban, so the soft injection is used there instead.
...
6. *(Optional)* Tick **Proactive Injection (learn & prevent)** to build a per-chat list of every detected phrase and have the model instructed to avoid them before every future reply. Open **Learned Phrases (this chat)** to review and hand-edit the list — add phrases of your own or delete ones you don't want injected. Adjust the injection **Depth**/**Role**, edit the **Proactive Prompt Template** (must keep `{{bannedPhrases}}`), and use **Clear Learned Phrases** to reset the chat's list. For a "prevent, don't rewrite" workflow, combine it with **Max Rewrite Attempts = 0**.
7. *(Optional, Text Completion only)* Also tick **Hard Token Ban** to feed the learned list into your backend's sampler-level `banned_strings`, so the model is *unable* to emit those phrases rather than just asked not to. On supported backends this replaces the soft injection; on Chat Completion APIs it has no effect and the injection is used instead.
...
| Hard Token Ban (Text Completion backends) | Requires Proactive. On Text Completion backends, appends the learned list to the request's sampler-level `banned_strings` (a hard ban the backend enforces), non-destructively per-request. Replaces the soft injection on those backends; no-op on Chat Completion APIs (default off) |
| Depth / Role | Where (how many messages from the bottom) and with which role the proactive instruction is injected |
| Learned Phrases (this chat) | The per-chat running list, one phrase per line. Auto-populated from detections and freely editable — add your own or delete entries. Stored with the chat, not globally |
| Clear Learned Phrases | Forget every phrase learned in the current chat and remove its proactive injection. A live count is shown alongside |
