...
- **Voicing walk-ons** — when enabled, detected walk-on characters join the director's speaker roster and the confirm/override dialog alongside real members (walk-ons shown with a dashed border). If the director picks (or you override to) a walk-on, it generates a reply in that character's voice and posts it under their name. By default this uses **native swipe-regeneration** (a placeholder is posted and SillyTavern's own swipe pipeline fills it in), so formatting and stop strings are handled natively. Turn that off to use the director's own improv generation from an editable template instead (the reply is then trimmed to the walk-on's single turn) — and a native attempt that fails falls back to it automatically.
...
| Voice walk-ons via native swipe-regeneration | Generate a chosen walk-on's reply through SillyTavern's own swipe pipeline (post a placeholder, then regenerate it) for native formatting/stop strings. Off = use the improv template below. Falls back to the template on failure (default on) |
| Walk-on Reply Template | The prompt used to generate a chosen walk-on's reply when native swipe-regeneration is off or fails. Macros: `{{name}}` (the walk-on), `{{context}}` (chat/character/lore preamble) |
