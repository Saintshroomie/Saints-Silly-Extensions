...
- **Prompt any earlier moment** — Every chat message also gets its own <span title="image icon">🖼</span> button (next to the per-message quick actions, toggleable in settings). Clicking it opens the same modal *anchored at that message* — the chat context fed to the LLM ends there, so the generated prompt depicts that moment of the story instead of the latest one — leaving you free to add Guidance or switch presets before clicking **Generate** (an optional setting starts the generation immediately instead). An anchor bar at the top of the modal shows which message the context ends at; click **Full Chat** on it to drop the anchor and go back to the current moment. The anchor holds for every Generate/Continue until you drop it or close the modal.
...
- **Negative prompt, per model** — A **Negative Prompt** field under the output holds what should be kept *out* of the image, and is sent along whenever you click Generate Image. It is part of each **Prompt Preset**, so every diffusion-model family carries the negative that model actually wants — the booru presets (**Anima**, **Danbooru Tags**) ship with the usual quality/anatomy tags, while the Krea 2 **Default** ships empty on purpose, since modern flow models barely respond to booru-era negatives. Edit it freely for a one-off shot; **Reset** puts it back under the preset's control and **Clear** empties it. It is added *in front of* the negative prompt in SillyTavern's own Image Generation panel rather than replacing it, so your global negatives keep applying. Saved prompts remember the negative they were saved with.
- **Render it without leaving SillyTavern** — A **Generate Image** button beside the output hands the finished prompt straight to SillyTavern's own **Image Generation** extension, which renders it on whichever backend you already configured there — **ComfyUI**, AUTOMATIC1111, SD.Next, DrawThings, NovelAI, Stable Horde, or any of the online APIs — and posts the result into the chat. The prompt is sent exactly as written (no second LLM pass rewriting it), with ST's own common prompt prefix and negative prompt applied on top, so your existing quality tags, resolution, workflow, and sampler settings all still apply. Every saved prompt gets the same button, so you can re-render an old prompt without loading it into the editor first. The button hides itself if SillyTavern's Image Generation extension is disabled, and it can be switched off in settings.
...
2. Click **Image Prompt** in the hamburger (options) menu next to the input field, or run `/imageprompt`. (Illustrating an earlier scene? Click the <span title="image icon">🖼</span> button on that message instead — the modal opens anchored there.)
...
5. Check the **Negative Prompt** field (it follows your preset — edit it for this shot, or **Reset** it back).
6. Click **Generate Image** to render it right there on your configured SillyTavern image backend (ComfyUI etc.) — or click **Copy & Close** (or the **Copy** button) and paste the prompt into the image tool of your choice.
7. Want to keep it? Click **Save**, give it a title (or accept the suggested one) — the prompt is stored with the chat, and the **Saved Prompts** section lets you reload, copy, rename, or delete it any time you reopen the modal in that chat.
8. Rendering for a different model? Switch the **Prompt Preset** right in the modal (e.g. to **Anima (Tags + Prose)** or **Danbooru Tags**) and generate again.
...
| Per-message 🖼 button | Add an image button to every chat message that opens the modal anchored at that message — the chat context ends there, so the prompt depicts that moment. On by default |
| Auto-generate on per-message button | Start a Generate as soon as the per-message button opens the modal. Off by default, so there's time to add Guidance or switch presets first |
| Generate Image button | Show the **Generate Image** button in the modal (and on every saved prompt), which renders the prompt on the backend configured in SillyTavern's own **Image Generation** extension — ComfyUI, A1111, NovelAI, and so on. On by default; automatically hidden when that extension is disabled |
| Don't post generated images to the chat | Generate the image without adding it as a chat message (it is still saved to the character gallery). Off by default — posting to chat matches ST's own `/imagine` |
...
| Negative Prompt | The negative prompt sent with **Generate Image** (not sent to the LLM). Part of the preset, so each diffusion-model family carries its own; seeds the editable Negative Prompt field in the modal. Added in front of SillyTavern's own negative prompt, which still applies |
