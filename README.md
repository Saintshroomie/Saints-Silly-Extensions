...
- **Proactive injection (avoid learned phrases)** — Optionally, instead of only fixing replies after the fact, Phrase Ban persistently injects a *"don't reuse these phrases"* instruction built from the learned list before every AI turn — so future replies avoid the tics up front. The injection's depth and role are configurable. Pair it with **Max Rewrite Attempts = 0** for pure prevention (collect + warn, never rewrite). When it's off, the list still builds but is never injected, and Phrase Ban behaves exactly as the reactive rewrite path.
...
| Proactive Injection (avoid learned phrases) | When on, injects a "don't reuse these phrases" instruction built from the Learned Phrases list before every AI turn. The list is built automatically regardless; this only controls whether it's injected. When off, Phrase Ban only fixes replies after the fact (default off) |
| Hard Token Ban (Text Completion backends) | Requires Proactive Injection. On Text Completion backends, appends the Learned Phrases list to the request's sampler-level `banned_strings` (a hard ban the backend enforces), non-destructively per-request. Replaces the soft injection on those backends; no-op on Chat Completion APIs (default off) |
