...
function openModal() {
    ...
    for (const [key, field] of getOrderedFields(currentSchema)) {
        fieldStates[key] = {
            originalSeed: '',
            currentValue: '',
            hasGenerated: false,
            prose: field.defaultProse ?? false,
            tokenOverride: null,
        };
    }
...
function buildModalHTML(schema, existingDesc) {
    ...
    for (const [key, field] of fields) {
        const isProse = field.defaultProse ?? false;
        const inputTag = isProse
            ...
            <div class="acc-field-group" data-field-key="${key}">
                <div class="acc-field-header">
                    ...
                    <div class="acc-field-controls">
                        <label class="acc-prose-toggle" title="Toggle prose mode (textarea vs text field)">
                            <input id="acc_prose_${key}" type="checkbox" ${isProse ? 'checked' : ''} />
                            <span class="acc-prose-label">Prose</span>
                        </label>
                        <label class="acc-token-label" title="Max tokens for generation (override)">
...
function swapInputType(fieldKey, toProse) {
    const wrap = document.getElementById(`acc_input_wrap_${fieldKey}`);
    const oldInput = document.getElementById(`acc_field_${fieldKey}`);
    if (!wrap || !oldInput) return;

    const currentValue = oldInput.value;
    const field = currentSchema.fields[fieldKey];
    const placeholder = escapeAttr(field?.description || '');
    ...
    newInput.id = `acc_field_${fieldKey}`;
    newInput.className = 'acc-field-input text_pole';
    newInput.placeholder = placeholder;
    newInput.value = currentValue;

    // Re-attach input listener
    newInput.addEventListener('input', () => {
        fieldStates[fieldKey].currentValue = newInput.value;
    });

    wrap.replaceChild(newInput, oldInput);
}
