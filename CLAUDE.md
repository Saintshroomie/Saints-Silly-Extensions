...
10. **Group Director** (`src/director.js`) — An LLM-chosen turn order for group chats. ST's `generateGroupWrapper` exposes no hook to override speaker selection, so the Director instead flips the active group to **Manual** reply order (`group.activation_strategy = MANUAL` via `editGroup`, capturing the previous strategy in `chatMetadata.director.previousStrategy` for restore) so ST stops auto-replying, then after each user turn's group wrapper settles (`GROUP_WRAPPER_FINISHED`, gated on `type === 'normal'` + the last message being `is_user`) runs a small silent "director" generation (`streamingGenerate` into a detached scratch element, tiny `responseLength`) that picks the next speaker from a numbered roster of un-muted members. (It fires on the wrapper-finished event, not `MESSAGE_SENT`, because the latter fires mid-pipeline while members are still being unshallowed — the roster would come back empty.) The pick is parsed (index → exact name → contained name, random fallback) and the chosen member is triggered with ST's native `context.generate('normal', { force_chid })`. An optional confirm/override dialog (a `Popup` with a button per cast member; disable via `directorConfirm`) lets the user accept or redirect. **Muted** members (`group.disabled_members`) are excluded; if the pick is the **possessed** character the Director yields silently (no dialog, no trigger). Manual roll via `/next` (alias `/director`). No `setExtensionPrompt` injection and no turn counter — simpler than NG.
...
- **Group Director**: only works in group chats — guard everything on `ctx.groupId`. The **Manual-mode flip must be proactive** (on enable + `CHAT_CHANGED`), not at send time, or ST auto-replies before the director runs; `applyManualMode`/`restoreStrategy` capture/restore `chatMetadata.director.previousStrategy` and self-heal on chat change when the Director is off. The `busy` flag fences only the decision phase (roll + dialog); the triggered `force_chid` reply is fire-and-forget so it never holds the wrapper-finished emit / dialog open. The auto trigger is `onDirectorGroupWrapperFinished`, which only rolls when the **last** message is `is_user` — that both restricts it to genuine user turns and prevents a loop (a triggered member reply leaves a character message as last, so it never re-rolls), and it naturally skips a possessed send (Possession makes the last message a character one). A `setTimeout(0)` lets ST's `Generate` stack unwind before the roll. Do **not** move this back to `MESSAGE_SENT` — that fires mid-pipeline (empty roster while members unshallow). Test: a group with 3+ members (roll + confirm + override), the confirm toggle off (auto-fire), `/next` chaining, a **muted** member (never offered), a **possessed** member (rolled or overridden → silent yield), disable mid-session (strategy restored), reload with it enabled (group stays Manual; self-heal restores when disabled), and a solo chat (inert). `force_chid` is an index into `context.characters` — validate the member resolves.
