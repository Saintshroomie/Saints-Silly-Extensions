...
6. **Reformatting** (`src/reformatting.js`) — Normalizes the formatting of AI character messages after generation. Two selectable engines: deterministic Rules (strip asterisks / wrap narration in asterisks / collapse whitespace) and LLM (editable `{{message}}` prompt + prefill). Auto (on `MESSAGE_RECEIVED`) + manual (a per-message button injected into `.mes_buttons`, kept present by a `#chat` MutationObserver). Preserves the original as a swipe and guards per-swipe so it never re-processes its own output.
