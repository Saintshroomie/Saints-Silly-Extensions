...
- **Group Director**: only works in group chats — guard everything on `ctx.groupId`. The **Manual-mode flip must be proactive** (on enable + `CHAT_CHANGED`), not at send time, or ST auto-replies before the director runs; `applyManualMode`/`restoreStrategy` capture/restore `chatMetadata.director.previousStrategy` and self-heal on chat change when the Director is off. The `busy` flag fences only the decision phase (roll + dialog); the triggered `force_chid` reply is fenced by ST's own `isGenerating`, and the trigger is **fire-and-forget** so it never holds the `MESSAGE_SENT` emit / dialog open. `onDirectorMessageSent` reacts only to `is_user` messages (Possession rewrites the send into a character message). Test: a group with 3+ members (roll + confirm + override), the confirm toggle off (auto-fire), `/next` chaining, a **muted** member (never offered), a **possessed** member (rolled or overridden → silent yield), disable mid-session (strategy restored), reload with it enabled (group stays Manual; self-heal restores when disabled), and a solo chat (inert). `force_chid` is an index into `context.characters` — validate the member resolves.
