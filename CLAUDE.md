...
- **Silent Generation** (`src/silent-generation.js`) — Central cancellation manager for background `generateRaw` calls. Hooks `GENERATION_STOPPED`, hands each job an AbortController, and exposes `abortAllGenerations()` so extension Stop buttons actually halt the LLM backend.
- **Prompt Templates** (`src/prompt-templates.js`) — Named, reusable presets (dropdown + Save/Update/Rename/Delete) for every editable prompt in the settings panel. Persisted in extension settings under `promptTemplates` / `activePromptTemplate`.
...
src/
  ...
  prompt-templates.js           Named prompt-template presets widget
...
- **Silent generations go through the manager.** ACC, WIA, and NG call `streamingGenerate` (in `utils.js`), which routes through `cancellableStreamingGenerate` in `silent-generation.js`. On cancel the awaiting caller gets an AbortError immediately; check it with `isSilentGenerationAbort(err)` to suppress error toasts. Extension Stop buttons must call `abortAllGenerations()` (not just the local abort) so ST's `GENERATION_STOPPED` fires and the backend fetch is actually cancelled.
...
- **Settings panel** is a single HTML blob (`src/settings.html`) injected into `#extensions_settings`, organized as per-tool collapsible drawers plus a Diagnostics drawer for all debug toggles. Each module owns its own `bind…Settings(saveSettings)` to attach listeners after injection. Editable prompts get a template widget via `setupPromptTemplates(...)`.
...
- **Settings panel**: when adding a setting, update `defaultSettings` in `index.js`, add the control to `settings.html`, and bind it in the relevant module's `bind…Settings`. For a new editable prompt, also add a `promptTemplates` / `activePromptTemplate` default key and a `setupPromptTemplates` call.
