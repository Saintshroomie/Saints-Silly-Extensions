...
3. **Phrase Ban** (`src/phrase-ban.js`) — Regex ban list for AI replies. Scans each incoming AI message against user-defined regex patterns and, on a match, rewrites it through the Phrasing engine (`rewriteMessageWithTemplate`) with the matched phrases listed in the prompt, retrying up to a configurable cap (0 = notify only). Auto (detached from `MESSAGE_RECEIVED`) + manual (`/phraseban`). Every rewrite is a swipe. Detection always accumulates the matched phrases into a per-chat learned list (`chatMetadata.phraseBan.bannedPhrases`, editable in a textarea) as bookkeeping — independent of whether the list is then *used*. An optional **Proactive Injection** toggle persistently injects a "don't reuse these phrases" instruction built from that list (`setExtensionPrompt`, key `phrase_ban_proactive`) before every AI turn; an optional **Hard Token Ban** instead appends the list to the request's `banned_strings` on Text Completion backends (via `TEXT_COMPLETION_SETTINGS_READY`, non-destructive) and suppresses the soft injection there. With both off the module behaves exactly as the reactive path — the list still builds but is never injected or banned.
