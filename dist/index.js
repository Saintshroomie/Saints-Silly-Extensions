...
function applyRulesReformat(text) {
    let out = text;
    if (reformatting_moduleSettings.reformattingStripAsterisks || reformatting_moduleSettings.reformattingWrapNarration) {
        out = stripAsterisks(out);
    }
    if (reformatting_moduleSettings.reformattingWrapNarration) {
...
function bindReformattingSettings(saveSettings) {
    ...
    const bindRuleToggle = (id, key) => {
        ...
        if (!cb) return;
        cb.checked = !!reformatting_moduleSettings[key];
        cb.addEventListener('change', () => {
            reformatting_moduleSettings[key] = cb.checked;
            saveSettings();
        });
    };
