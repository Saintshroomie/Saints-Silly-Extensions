...
let director_possessionApi = null;
let director_debug = () => {};
...
function isPossessedMember(member) {
    if (!member || !director_possessionApi?.isPossessing?.()) return false;
    const possessed = director_possessionApi.getPossessedCharacter?.();
    if (possessed) {
    ...
    const possessedName = director_possessionApi.getPossessedCharName?.();
...
function parsePick(text, roster) {
    ...
    director_debug('No roster match for director reply; choosing at random. Reply was:', text);
...
async function runDirector({ manual = false } = {}) {
    ...
    try {
        ...
        if (isPossessedMember(suggested)) {
            director_debug('Suggested speaker is the possessed character — yielding to the user.');
            return;
        }
        ...
        if (director_moduleSettings.directorConfirm) {
            ...
            if (isPossessedMember(chosen)) {
                director_debug('Override is the possessed character — yielding to the user.');
                return;
            }
        }
 ...
 */
function initDirector({ settings, possessionApi: possession }) {
    director_moduleSettings = settings;
    director_possessionApi = possession || null;
...
jQuery(async () => {
    ...
    initDirector({
        settings: src_settings,
        possessionApi: { isPossessing: isPossessing, getPossessedCharacter: getPossessedCharacter, getPossessedCharName: getPossessedCharName },
    });
