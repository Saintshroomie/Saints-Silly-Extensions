...
import { is_group_generating as __WEBPACK_EXTERNAL_MODULE__group_chats_js_678c16bd_is_group_generating__ } from "../../../../group-chats.js";
 ...
 */
function initPhraseBan({ settings, phrasingApi: pApi }) {
...
function shouldUseHardBan() {
    return !!(phrase_ban_moduleSettings?.phraseBanEnabled
        && phrase_ban_moduleSettings.phraseBanProactive
        && phrase_ban_moduleSettings.phraseBanHardBan
...
function reapplyProactiveInjection() {
    ...
    if (shouldUseHardBan()) {
        clearProactiveInjection();
        phrase_ban_debug('Proactive injection suppressed — hard token ban active for this backend');
        return;
    }
...
function onPhraseBanTextCompletionSettings(params) {
    ...
    if (added) phrase_ban_debug('Hard ban — appended', added, 'learned phrase(s) to banned_strings');
}
 ...
 */
function onPhraseBanGenerationStarted() {
    reapplyProactiveInjection();
}
    ...
    try {
        ...
        for (let attempt = 0; ; attempt++) {
            ...
            if (index !== context.chat.length - 1) {
                phrase_ban_debug('scan — message is no longer the last in the chat; cannot rewrite');
            ...
            const result = await phrase_ban_phrasingApi.rewriteMessageWithTemplate(
                index,
                getPromptTemplate(),
            ...
            if (!result) {
                phrase_ban_debug('scan — rewrite did not run or returned empty; stopping');
                return;
            }
            await delay(SETTLE_DELAY_MS);
        }
...
function bindPhraseBanSettings(saveSettings) {
    ...
    if (hardBanCb) {
        hardBanCb.checked = !!phrase_ban_moduleSettings.phraseBanHardBan;
        hardBanCb.addEventListener('change', () => {
            phrase_ban_moduleSettings.phraseBanHardBan = hardBanCb.checked;
            saveSettings();
