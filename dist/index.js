 ...
 */
function onToolPresetChange(toolKey, listener) {
    const tool = getTool(toolKey);
 ...
 */
async function sendPromptToImageGen(prompt, { quiet = false, negative = '' } = {}) {
...
const DEFAULT_IMAGE_PROMPT_NEGATIVE = '';
...
const ANIMA_IMAGE_PROMPT_NEGATIVE =
...
const DANBOORU_IMAGE_PROMPT_NEGATIVE =
...
function loadSavedPrompt(id) {
    ...
    if (typeof entry.negative === 'string') {
        ...
        if (negative) {
            ...
            image_prompting_persistedModalState.negative = entry.negative;
        }
    }
...
function currentPresetNegative() {
    return (typeof image_prompting_moduleSettings?.imagePromptNegative === 'string')
        ? image_prompting_moduleSettings.imagePromptNegative
        : DEFAULT_IMAGE_PROMPT_NEGATIVE;
}
...
function resolveNegativeForField() {
    ...
    const saved = image_prompting_persistedModalState.negative;
...
function setNegativeFromPreset() {
    ...
    field.value = preset;
    seededNegative = preset;
    image_prompting_persistedModalState.negative = preset;
}
...
function onPresetChangedRefreshNegative() {
    ...
    setNegativeFromPreset();
    image_prompting_debug('Negative prompt re-seeded from the newly activated preset');
}
