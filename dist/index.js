...
function getAutoPhrasingInput() {
    if (!phrasing_ctx.settings.phrasingEnabled || !phrasing_ctx.settings.phrasingAutoEnabled) return null;
...
async function runAutoPhrasing(inputText) {
    ...
    phrasing_debug('runAutoPhrasing — starting, possessed:', possessing, '| input length:', inputText.length);
    ...
    try {
        ...
        if (possessing) {
            ...
            if (autoPhrasingStopped) {
                phrasing_debug('runAutoPhrasing — stopped by the user; reply not triggered');
                return;
            }
        ...
        if (autoPhrasingStopped) {
            ...
            phrasing_debug('runAutoPhrasing — stopped by the user; partial rewrite left in the chat box');
            return;
        }

        phrasing_debug('runAutoPhrasing — sending rewrite, length:', result.length);
        submitCurrentInput();
    } catch (err) {
        ...
        showAllPhrasingButtons();
        phrasing_debug('runAutoPhrasing — complete');
    }
}
...
function submitCurrentInput() {
    ...
    if (!sendButton) {
        phrasing_debug('submitCurrentInput — no send button found');
        return;
    }
    sendButton.click();
}
 ...
 */
function attachAutoPhrasingInterceptor() {
    document.addEventListener('click', (event) => {
        ...
        phrasing_debug('Intercepted send button for Auto Phrasing');
    ...
    document.addEventListener('keydown', (event) => {
        ...
        phrasing_debug('Intercepted Enter for Auto Phrasing');
        runAutoPhrasing(inputText);
    }, { capture: true });

    phrasing_debug('Attached Auto Phrasing send interceptor');
}
...
function injectCopyButton(formEl) {
    if (!world_info_assist_moduleSettings?.wiaCopyButtonEnabled) return;
...
async function onCopyEntryContent(formEl) {
    ...
    await copyTextToClipboard(text, {
        ...
        debug: world_info_assist_debug,
    });
    world_info_assist_debug('Copied entry content, length:', text.length);
}
...
function getImagineCommand() {
    const commands = __WEBPACK_EXTERNAL_MODULE__slash_commands_SlashCommandParser_js_42c8b851_SlashCommandParser__.commands;
 ...
 * @returns {string}
 */
function getImageGenSourceLabel() {
 ...
 */
function checkImageGenConfigured() {
 ...
 */
async function sendPromptToImageGen(prompt, { quiet = false } = {}) {
...
function imageGenButtonEnabled() {
    return image_prompting_moduleSettings?.imagePromptSendToImageGenEnabled !== false && isImageGenAvailable();
}
...
async function dispatchImageGen(text, label) {
    isSendingImage = true;
    image_prompting_debug('Sending prompt to image generation:', label, `${text.length} chars`);
    try {
        const url = await sendPromptToImageGen(text, {
            quiet: !!image_prompting_moduleSettings?.imagePromptImageGenQuiet,
        });
        if (url) {
            toast(`Image generated on ${label}.`, 'success');
            image_prompting_debug('Image generated:', url);
        } else {
            // ST already raised its own toast (backend unreachable, stopped
            // mid-generation, bad workflow) — don't stack a second one.
            image_prompting_debug('Image generation returned no URL; ST reported the reason.');
        }
    } catch (err) {
 ...
 * @returns {boolean} `true` if anything changed (caller should save settings).
 */
function migrateDirectorPrompt(settings) {
...
async function generateAndPostWalkOn(ctx, name) {
    ...
    try {
        ...
        if (!text || text === '…') {
            ...
            director_debug('Native walk-on swipe produced nothing for', name);
            return;
        }
...
async function runDirector({ manual = false, turns } = {}) {
    ...
    try {
        for (let turn = 0; turn < total; turn++) {
            ...
            if (!group) {
                director_debug('runDirector stopping — no active group object');
                break;
            }

            const roster = buildRoster(turnCtx, group);
            director_debug(`Turn ${turn + 1}/${total} roster:`, roster.map(m => m.name));
            if (!roster.length) {
                if (manual && turn === 0) toast('Group Director: no eligible (unmuted) characters to choose from.', 'warning');
                director_debug('runDirector stopping — empty roster');
                break;
            }
            ...
            if (!chosen) {
                director_debug('Director turn cancelled / no pick — stopping chain at turn', turn + 1);
...
function loadSettings() {
    ...
    if (migrateDirectorPrompt(src_settings)) {
...
function src_onGenerationStopped() {
...
jQuery(async () => {
    ...
    eventSource.on(eventTypes.GENERATION_STOPPED, src_onGenerationStopped);
