...
/* Diagnostic, not a story action — sits apart from the generate/continue set. */
.cc-dump-btn {
    flex: 0 0 auto;
    min-width: 0;
    opacity: 0.75;
...
async function dumpCompactionPrompt() {
    ...
    compaction_setStatusBar('Dumping prompt to console…');
    try {
        ...
        for (const s of sections) {
            let tokens = null;
            try {
                tokens = await __WEBPACK_EXTERNAL_MODULE__tokenizers_js_d5863f55_getTokenCountAsync__(s.text);
            } catch { /* token counting is best-effort here */ }
            sized.push({ ...s, tokens });
        }
        const systemTokens = await __WEBPACK_EXTERNAL_MODULE__tokenizers_js_d5863f55_getTokenCountAsync__(COMPACTION_SUMMARY_SYSTEM_PROMPT).catch(() => null);
        const promptTokens = await __WEBPACK_EXTERNAL_MODULE__tokenizers_js_d5863f55_getTokenCountAsync__(prompt).catch(() => null);
